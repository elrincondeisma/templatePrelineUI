<!-- ========== HEADER ========== -->
<header class="sticky top-0 inset-x-0 flex flex-wrap md:justify-start md:flex-nowrap z-[48] w-full bg-white border-b text-sm py-2.5 lg:ps-[260px] dark:bg-gray-800 dark:border-gray-700">
    ...
    <div class="w-full flex items-center justify-end ms-auto md:justify-between gap-x-1 md:gap-x-3">
        ...
        <div class="relative">
            ...
            <button type="button" class="inline-flex shrink-0 justify-center items-center size-6 rounded-full text-gray-500 hover:text-blue-600 focus:outline-none focus:text-blue-600 dark:text-gray-500 dark:hover:text-blue-500 dark:focus:text-blue-500" aria-label="Close">
        ...
        <div class="hs-dropdown [--placement:bottom-right] relative inline-flex">
            <button id="hs-dropdown-account" type="button" class="size-[38px] inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-full border border-transparent text-gray-800 focus:outline-none disabled:opacity-50 disabled:pointer-events-none dark:text-white" aria-haspopup="menu" aria-expanded="false" aria-label="Dropdown">
                <span class="inline-flex items-center justify-center size-8 rounded-full bg-gray-500 text-xs font-semibold text-white leading-none">
                    @php
                        $nombreCompleto = Auth::user()->name; // Aquí asumes que el campo 'name' contiene el nombre completo
                        $arrayNombreCompleto = explode(' ', $nombreCompleto);
                        // Asumiendo que el campo 'apellido' contiene el apellido
                        $iniciales = substr($arrayNombreCompleto[0], 0, 1) . substr($arrayNombreCompleto[1], 0, 1);
                    @endphp
                    {{ $iniciales }}
                  </span>
            </button>
            ...
            <div class="p-1.5 space-y-0.5">
                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700 dark:focus:text-gray-300" href="{{ route('profile') }}">
                    <x-heroicon-o-user class="shrink-0 size-4" aria-hidden="true" />
                    {{ __('Profile') }}
                </a>
                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700 dark:focus:text-gray-300" href="{{ route('logout') }}">
                    <x-heroicon-o-arrow-left-end-on-rectangle class="shrink-0 size-4" aria-hidden="true" />
                    {{ __('Log Out') }}
                </a>
